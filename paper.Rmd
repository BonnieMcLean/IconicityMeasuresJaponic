...
Iconicity in language is receiving increased attention from many fields, but our understanding of the roles of iconicity in language is only as good as the measures we use to quantify it. We conducted iconicity rating and guessing experiments with 300 Japanese ideophones and prosaic words with similar meanings, (e.g. *fuwafuwa* ‘fluffy’, *jawarakai* ‘soft’), and found that the two measures together presented a more complete picture of iconicity than either alone. For both groups of words, ratings and guesses were positively correlated—emphasising that both pick up on the same form-meaning associations. Ideophones were consistently associated with higher iconicity ratings, but not higher guessing accuracy. We suggest that the structural markedness of ideophones enhances their perceived iconicity in the rating task, but does not provide any advantage (over and above that provided by form-meaning associations) in the guessing task. Thus, guesses and ratings could be used together to tease apart the relative contribution of structural markedness to iconic effects. Some ideophones were also guessed very poorly, highlighting that construals of iconicity can be language-specific. Combining guesses with ratings from native speakers is thus recommended for a fuller picture of language-external and language-internal mappings (Occhino et al. 2017).

We also critically evaluated different rating and guessing paradigms, and introduce a new guessing paradigm that improves on the robustness, sensitivity and discriminability of previous approaches. Finally, we introduce a reproducible workflow for creating rating and guessing experiments in the form of a Python package, `icotools`, which we hope will improve comparability between future studies, enabling others to confirm and build on current findings, and to ask new questions about the roles of iconicity in language.
...
In iconicity rating tasks, the concept of iconicity is explained to participants, and participants are asked to rate the iconicity of given form-meaning pairs directly [e.g. @vinson_british_2008; @perry_iconicity_2015; @winter_which_2017; @occhino_iconicity_2017; @thompson_iconicity_2020; @punselie_iconicity_2020]. Measurements from these studies are shown to correlate in meaningful ways with factors related to iconicity--e.g. encoding of sensory information [@winter_which_2017], semantic neighbourhood density [@sidhu_lonely_2018], age of acquisition [@vinson_british_2008; @perry_iconicity_2015], facilitated processing [@thompson_link_2009; @occhino_role_2020], word class [@perry_iconicity_2015; @thompson_iconicity_2020], semantic domain [@punselie_iconicity_2020], funniness and structural markedness [@dingemanse_playful_2020]. However, ratings given are strongly influenced by the linguistic experience of participants (@occhino_iconicity_2017; @sevcikova_sehyr_perceived_2019), which could make comparing measurements between studies difficult.
...
ratings%>%
  filter(form=="pyoNpyoN"|form=="katai"|form=="hisohiso"|form=="yoroyoro"|form=="fuwafuwa")%>%
  select(experiment,prolificID,form,rating,taskdesc)->ratings_examine

#ratings_examine%>%kable()%>%kable_styling()
```
...
The foil words were artificially generated to be as phonologically distinct as possible from the answer word, while still conforming to Japanese phonology. For every word, three different foil words were generated by substituting each consonant with its top three most phonologically distant consonants in the Japanese sound system. Phonological distances were calculated using the Phonological Corpus Tools software [@hall_phonological_2019], with the feature system provided by Mielke [-@mielke_phonetically_2012]. Vowels were substituted by rotating the vowel space 180 degrees, so that /a/ > /u/, /i/ > /a/, /u/ > /i/, /e/ > /o/, and /o/ > /e/. Finally, reduplicated words were ‘unreduplicated’ in their foil words. For example, the foils for *fuwafuwa* were *rizyu*, *gityu* and *nitsu* (not *rizyurizyu*, *gityugityu* and *nitsunitsu*). 
...
Participants had to listen to the Japanese words and were told their meanings, then asked to rate the resemblance between the word and its meaning on a scale from 0 'No resemblance' to 6 'Strong resemblance'. Previous studies [@perry_iconicity_2015] also used a negative end of the scale (corresponding to a 'bad resemblance'). However, follow-up analysis found that the negative end of the scale was both underused and inconsistently used, suggesting that anti-iconic relationships are difficult to assess [@motamedi_iconicity_2019]. For this reason, we decided to only use a positive scale in this study. A sample trial is shown in Figure \@ref(fig:ratingdemo).
...
ratings%>%
  ...
  add_count(trans)%>%
  filter(n>=10)%>%
  select(identifier,trans,rating)->data
